<template>
  <div>
      ...
      <table :class="tableClasses">
        <thead>
          <tr v-if="header">
            <th width="50px" class="text-center">
              ...
              <b-form-checkbox label="" v-model="itemdata"></b-form-checkbox>
            </th>
              ...
              <th :class="headerClass(index)" :key="index" style="vertical-align:top;">
                <slot :name="`${rawColumnNames[index]}-filter`">
                  <select :ref="colName"
                    ...
                    <option v-for="(opt,idx) in fields[index].filter_select_options"
                            :value="opt.key">
                      {{opt.value}}
                    </option>
                  </select>
                  <b-input-group v-if="!fields || (fields[index].filter !== false && fields[index].filter_number)"
                                 size="sm">
                    <template v-slot:prepend>
                      <b-input-group-text>Min</b-input-group-text>
                    </template>
                    <input class="form-control form-control-sm filter-column"
                  ...
                  <b-input-group v-if="!fields || (fields[index].filter !== false && fields[index].filter_date)"
                                 size="sm">
                    <input type="date"
                  ...
                  <input v-if="!fields || (fields[index].filter !== false
        ...
        <tbody :style="clickableRows ? 'cursor:pointer;': null" class="position-relative">
          <template v-for="(item, itemIndex) in currentItems">
            <tr :class="item._classes">
              <td style="vertical-align:top;" class="text-center">
                <a href="javascript:;"
                   style="border-radius:40%;text-decoration:none;">
                  <!--<b-form-checkbox label="" value=true unchecked-value=false @click.native="checkRole(itemIndex)" v-model="itemdata"></b-form-checkbox>-->
                  <b-form-checkbox label=""  v-model="itemdata"></b-form-checkbox>
                </a>
    ...
    <div right style="float:right;">
      <b-button variant="blue" type="submit" @click="handleSelectButton">&nbsp;Select</b-button>
    </div>
    ...
    <div class="row">
      <b-col md="8">
        <b-pagination v-if="pagination"
                      ...
                      @change="pagingChange"></b-pagination>
      </b-col>
      <b-col md="4">
        <div v-if="itemsPerPageSelect || tableFilter" class="row mx-0">
          <div v-if="itemsPerPageSelect"
               class="col-sm-6 p-0"
            ...
            <select class="small" @change="paginationChange">
              ...
              <option v-for="(number, key) in paginationSelect.values"
                      :val="number"
                      :key="key">
                {{number}}
              </option>
...
export default {
  ...
  methods: {
    ...
    handleSelectButton(item) {
      ...
      this.actSelectRow(this.currentItems);
    }
  }
};
</script>
