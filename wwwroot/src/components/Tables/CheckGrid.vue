<template>
  <div>
      ...
      <table :class="tableClasses">
        <thead>
          <tr v-if="header">
            <th width="50px" class="text-center">
              ...
              <b-form-checkbox v-model="isCheckedAll" @change="onCheckAll($event)"></b-form-checkbox>
            </th>
          ...
          <tr v-if="columnFilter" class="table-sm">
            <th style="vertical-align:top; max-width:200px;" class="text-center">
              <!--ACTIONS-->
              <!--b-button variant="outline-danger" size="sm"><font-awesome-icon :icon="['fas', 'trash']" @click="clearFilter"/></b-button -->
              <a
                href="javascript:;"
                style="text-decoration:none;color:#ff8a80;"
                v-if="onFiltering"
                @click="clearFilter"
              >
                <font-awesome-icon :icon="['fas', 'times']" variant="danger" />
              </a>
            </th>
            <template v-for="(colName, index) in rawColumnNames">
              <th :class="headerClass(index)" :key="index" style="vertical-align:top;">
                <slot :name="`${rawColumnNames[index]}-filter`" class="text-center">
                  <select
                    :ref="colName"
        ...
        <tbody :style="clickableRows ? 'cursor:pointer;': null" class="position-relative">
          <template v-for="(item, itemIndex) in currentItems">
            <tr :class="item._classes" :tabindex="clickableRows ? 0 : null" :key="itemIndex">
              <td style="vertical-align:middle;" class="text-center">
                <b-form-checkbox v-model="item.IsChecked" @change="onCheckedChange(item, $event)"></b-form-checkbox>
              </td>
              <template v-for="(colName, index) in rawColumnNames">
                <slot
                ...
                />
                <td
                  v-if="fields[index].filter_date && !fields[index].currency"
                  :class="cellClass(item, colName, index)"
                  :key="index"
                >{{String(item[colName]) | formattedDMY(fields[index].date_format) }}</td>
                <td
                  v-if="fields[index].currency && !fields[index].filter_date"
                  :class="cellClass(item, colName, index)"
                  :key="index"
                >
                  {{
    ...
    <div class="row">
      ...
      <b-col md="1"></b-col>
      <b-col md="3" >
        <b-input-group right class="input-group-right">
          <template v-slot:prepend>
            <b-button
              size="md"
              :disabled="totalSelectedRecords<1"
              v-b-tooltip.hover
          ...
          <b-button right
            ...
            >
              <!--font-awesome-icon size="md" :icon="['fas', 'star']" class="text-orange" /-->
              <b-badge pill variant="warning">{{this.totalSelectedRecords}}</b-badge>&nbsp;
              <span class="text-bold">SELECTED RECORD(S)</span>
            </b-button>
        </b-input-group>
...
export default {
  ...
  props: {
    ...
    selectedItemArray: Array, //SELECTED BUCKET - must be refference
    selectedFieldName: ""
  },
  data() {
    return {
  ...
  watch: {
    ...
    items(val, oldVal) {
      ...
      this.passedItems = this.syncIncoming(val);
    },
    totalPages: {
      immediate: true,
      handler(val) {
  ...
  computed: {
    ...
    rows() {
      let totalRecord = this.items.length;
    ...
    columnFiltered() {
      let columnFilteredItems = this.passedItems;
      if (this.columnFilter && this.columnFilter.external) {
        return columnFilteredItems;
      }
      return columnFilteredItems;
    },
    filterableCols() {
    ...
    sortedItems() {
      ...
      if (!col || !this.rawColumnNames.includes(col) || this.sorter.external) {
        let passed = this.syncIncoming(this.items);
        return passed;
        //return this.items;
      }
      ...
      this.passedItems = this.syncIncoming(this.items);

      return this.passedItems.slice().sort((item, item2) => {
    ...
    paginationSelect() {
      return {
        label: this.itemsPerPageSelect.label || "Per-page:",
    ...
    totalSelectedRecords() {
      let count = 0;
      /*
      let count = this.dataItems.reduce(function(total, obj) {
          if(obj.IsChecked) 
            total++;
          return total;
      }, 0);*/
      if(this.dataItems != null && this.dataItems !== undefined){
  ...
  methods: {
    initCheckGrid() {
      ...
      const fieldKey = this.selectedFieldName;
      //let items = cloneDeep(this.sourceItems);
      let items = [];

      //SET SELECTED ITEMS ON INIT BY PASSING PARAMETERS

      if (this.selectedItemArray.length) {
        items = cloneDeep(this.selectedItemArray);
      }
    ...
    columnFilterEvent(colName, value, type) {
      ...
      //Needed to reset to page 1 on every filter changes
      this.currentPage = 1;
    ...
    cellButton(item, actionName, index) {
      let buttonValid = false;
      if (item.ActionButtons) {
        item.ActionButtons.forEach(function(actButton) {
          if (actButton === actionName) buttonValid = true;
          return;
        });
      }
    ...
    handleEditButton(props) {
      this.actEditRow(props);
    },
    handleDeleteButton(props) {
      this.actDeleteRow(props);
    },
    handleActionButton(item, action) {
      this.$eventBus.$emit(action.toLowerCase() + "-clicked", item, action);
    },
    //SYNC QUERY DATA WITH SOURCE
    //MUST SYNCHRONOUS TO MUTATE ARRAY
    syncIncoming(incoming) {
      let checkBoxAll = true;

      let arr = cloneDeep(incoming);
      let fieldKey = this.selectedFieldName;
      if (incoming.length > 0) {
        //console.log('[Sync Incoming]');
        let parentData = this.dataItems;

        arr.forEach(function(itm) {
          let found = parentData.filter(function(item) {
            return item[fieldKey] == itm[fieldKey];
          });
          if (found.length > 0) {
            //console.log('[syncIncoming] CHANGE CHECKED ' + found[0][fieldKey]);
            //console.log(found[0]);
            itm["IsChecked"] = true;
          } else {
            itm["IsChecked"] = false;
      ...
      this.isCheckedAll = checkBoxAll; //SET CHECKALL

      let val = JSON.parse(JSON.stringify(arr));

      return val;
    },
    removeArrayItemByIndex(arr, value) {
      let selectedFieldKey = this.selectedFieldName;
      var i = 0;

      while (i < arr.length) {
        if (arr[i][selectedFieldKey] === value) {
          arr.splice(i, 1);
        } else {
          ++i;
        }
    ...
    async onCheckedChange(row, checked) {
      let selectedFieldKey = this.selectedFieldName;
      let item = this.dataItems.filter(function(item) {
        return item[selectedFieldKey] == row[selectedFieldKey];
      });

      if (checked) {
        if (item.length <= 0) {
          this.dataItems = this.dataItems.concat(row);
        }
      } else {
        this.dataItems = this.removeArrayItemByIndex(
          this.dataItems,
          row[selectedFieldKey]
        );
      }
    },
    async onCheckAll(checked) {
      let selectedFieldKey = this.selectedFieldName;

      let myData = this.dataItems;

      let record = [];
      for (let i = 0; i < this.passedItems.length; i++) {
        record = this.passedItems[i];

        let item = myData.filter(function(item) {
          return item[selectedFieldKey] == record[selectedFieldKey];
        });
        if (checked) {
          if (item.length <= 0) {
            myData = myData.concat(record);
          }
        } else {
          myData = this.removeArrayItemByIndex(
            myData,
            record[selectedFieldKey]
          );
        }
      }

      this.dataItems = myData;
    },
    async onClearCart(checked) {
      this.dataItems = [];
    },
    handleSelectButton() {
      let datas = this.dataItems;

      if (this.actSelectRow != null) {
        this.actSelectRow(datas);
      }
    }
  }
};
