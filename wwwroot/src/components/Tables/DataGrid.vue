  ...
  <div>
    ...
    <div class="row">
      <b-col md="8" style="vertical-align:bottom;">
        <span class="table-title">List of {{caption}}</span>
      </b-col>
      <b-col md="4">
        <div class="form-inline justify-content-sm-end" v-if="!disableNewButton">
          <b-button variant="success" style="margin:2px;" size="sm" rounded @click="onCreateClick">
            <font-awesome-icon :icon="['fas', 'plus-circle']" />&nbsp;New
          </b-button>
        </div>
      </b-col>
    </div>
            ...
            >
              <td style="vertical-align:middle;" class="text-center">
                ...
                >
                  ...
                  <b-dropdown-item  @click="handleEditButton(item)" variant="blue">
                    <font-awesome-icon size="sm" :icon="['fas', 'edit']" />&nbsp;{{isEdit && !historical ?"Edit":"View"}}
                  </b-dropdown-item>
                  
                  <b-dropdown-item
                    v-if="cellButton(item,'ACTIONPRINT')"
                    @click="handleActionButton(item, 'ACTIONPRINT')"
                    variant="primary"
                  >
                    <font-awesome-icon size="sm" :icon="['fas', 'print']" />&nbsp;Print
                  </b-dropdown-item>
                  <b-dropdown-item
                    v-if="cellButton(item,'ACTIONVOUCHER')"
                    @click="handleActionButton(item, 'ACTIONVOUCHER')"
                    variant="primary"
                  >
                    <font-awesome-icon size="sm" :icon="['fas', 'print']" />&nbsp;Voucher
                  </b-dropdown-item>
                  <b-dropdown-item 
                    v-if="(isAllowDelete || cellButton(item,'DELETE')) && !historical"
                    @click="handleDeleteButton(item)"
                    variant="danger"
                  >
                    <font-awesome-icon size="sm" :icon="['fas', 'trash']" />&nbsp;Delete
                  </b-dropdown-item>
                  
                </b-dropdown>
                <!--<b-icon v-else icon="info"></b-icon>-->
              </td>
              <template v-for="(colName, index) in rawColumnNames">
                <slot
                ...
                <td
                  ...
                  :key="index"
                >
                  {{String(itemValue(item,colName)) | formattedDMY(fields[index].date_format) }}
                </td>
                <td
                ...
                >
                  {{
                    itemValue(item,colName) | currency({symbol: '', fractionCount: '2', fractionSeparator: ',', 
                ...
                <td
                  ...
                  :class="cellClass(item, colName, index)"
                  :key="index"
                >{{ String(itemValue(item,colName))  }}</td>
        ...
        <div v-if="itemsPerPageSelect || tableFilter" class="row my-2 mx-0">
          ...
          >
            <div class="form-inline justify-content-sm-end">
              <label class="mr-2 small">{{paginationSelect.label}}</label>
              <select class="small" @change="paginationChange" size="sm">
...
export default {
  ...
  props: {
    ...
    actEditRow: { type: Function },
    actDeleteRow: { type: Function },
    actDetail: { type: Function },
    actCreate: { type: Function },
  ...
  computed: {
    ...
    isAllowDelete() {
      if(this.isDelete && this.actDeleteRow != null){
        return true;
      }else{
        return false;
      }      
  ...
  methods: {
    ...
    cellClass(item, colName, index) {
      ...
      if(item._cellClasses){
        classes.push(item._cellClasses);
      }
      return classes;
    },
