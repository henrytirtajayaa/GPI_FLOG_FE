...
const configUrl = "GetProgressApplyPayable?"
...
export const state = () => ({
  ...
  listInfoUnapply: [],
  baseUrl: configUrl,
  baseUrlADV: "GetPendingAdvancePayment?",
  baseUrlCN: "GetPendingAPCreditNote?",
  baseUrlUnapply: "GetPendingAPUnapply?",
  headers: [
    {
      label: 'Doc. No',
      key: 'DocumentNo',
    ...
    try {
      var response = await new Axios({
        ...
        .post('ApplyPayable', {
          ...
          APApplyDetails : model.APApplyDetails
        })
        .then(response => {
  ...
  async actUpdate({ commit, rootState }, model) {
    try {
      var response = await new Axios({
        ...
        .put('ApplyPayable', {
          PayableApplyId : model.PayableApplyId,
          TransactionDate: model.TransactionDate,
          DocumentType: model.DocumentType,
          CheckbookTransactionId: model.CheckbookTransactionId,
          PayableTransactionId: model.PayableTransactionId,
  ...
  async actDelete({ commit, rootState }, model) {
    try {
      var response = await new Axios({
        ...
        .delete('ApplyPayable', { data: { PayableApplyId: model.PayableApplyId } })
        .then(response => {
