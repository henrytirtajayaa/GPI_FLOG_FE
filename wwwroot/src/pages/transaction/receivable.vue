              ...
              <b-card>
                <div slot="header">
                  ...
                  <div right style="float:right;">
                    <b-button variant="dark" v-if="form.model.DocumentNo != '' && actionButton.AllowVoid" 
                    @click="handlePrintOut" v-b-tooltip.hover title="View Print Out">
                      ...
                      <b-col sm="9">
                        <b-card sm="12" style="margin:0; padding:0;">
                          ...
                          <b-row>
                            ...
                            <b-col sm="4">
                              <label>SO Document No</label>
                              <b-form-input v-model="form.model.SoDocumentNo"
                                            class="font-weight-bold"
                                            size="sm"
                ...
                <b-row class="mb-2">
                  <b-col>
                    <b-button variant="yellow"
                              size="sm"
                              type="submit"
                              @click.prevent="onModalSearchCharges('ChargesCode','ChargesCode','receivable')"
          ...
          <b-col cols="12" sm="12">
            <b-card width="100%">
              <DataGrid ref="gridMain"
                        :fields="this.$store.state.features.finance.receivable.receivable_transaction.headers"
                        :items="this.$store.state.features.finance.receivable.receivable_transaction.data"
                        :info="this.$store.state.features.finance.receivable.receivable_transaction.listInfo"
                        :baseUrl="this.$store.state.features.finance.receivable.receivable_transaction.baseUrl"
                        ...
                        pagination
                        column-filter
                        caption="Receivable - Progress"></DataGrid>
            </b-card>
          </b-col>
        </b-row>
        <!-- TAB Customer END -->
            ...
            <b-col cols="12" sm="12">
              <b-card width="100%">
                <DataGrid ref="gridMain123"
                          :fields="this.$store.state.features.finance.receivable.receivable_history.headers"
                          :items="this.$store.state.features.finance.receivable.receivable_history.data"
                          :info="this.$store.state.features.finance.receivable.receivable_history.listInfo"
                          :baseUrl="this.$store.state.features.finance.receivable.receivable_history.baseUrl"
                          ...
                          :isDelete = "false"
                          :isEdit = "false"
                          caption="Receivable - History"></DataGrid>
              </b-card>
            </b-col>
          </b-row>
        </b-card-text>
...
<script>
  ...
  export default {
    ...
    async created(){
      ...
      await this.$store.dispatch(
        "features/finance/receivable/receivable_transaction/actGetData",
        "GetProgressReceivable?"
      );
      await this.$store.dispatch(
        "features/finance/receivable/receivable_history/actGetData",
        "GetHistoryReceivable?"
      );
    ...
    methods: {
      handlePrintOut(){
        let routeData = this.$router.resolve(
        {
          ...
          query: {
            'DocType': 'receivable', 
            'DocId' : this.form.model.ReceiveTransactionId, 
      ...
      async handleGet(url) {
        ...
        if (url.toLowerCase().lastIndexOf("GetHistoryReceivable") < 0) {
          url = "GetHistoryReceivable?" + url;
        }
        await this.$store.dispatch(
          "features/finance/receivable/receivable_history/actGetData",
          url
      ...
      async History() {
        this.itemHistory = this.$store.state.features.finance.receivable.receivable_history.data;
        this.HistoryInfo = this.$store.state.features.finance.receivable.receivable_history.listInfo;
      },
      ...
      async initDefaultForm() {
        const recSetup = cloneDeep(this.$store.state.features.company.receivable_setup
          .data);

        this.ReceivableSetups = recSetup;
        ...
        if (this.ReceivableSetups != undefined && Array.isArray(this.ReceivableSetups)) {
          this.form.defaultRateType = this.ReceivableSetups[0].DefaultRateType;
        }

        //BEGIN DOCUMENT TYPE & TRANSACTION TYPE
        let trxModule = this.$store.state.features.finance.receivable.receivable_transaction.TrxModule;
      ...
      async handleEdit(row) {
        ...
        row.ReceiveTransactionDetails.forEach(function (d, index) {
          arr.push({
            Index: index,
      ...
      async doDelete(row) {
        ...
        await this.$store.dispatch(
          "features/finance/receivable/receivable_transaction/actPutTrxDelete",
          row
        );

        var response = this.$store.state.features.finance.receivable
          .receivable_transaction.resultTrxDelete;
        if (response.status != 200) {
          this.handleToast(
            "Confirmation",
            "danger",
      ...
      async onSubmit() {
        if (this.validateForm()) {
          ...
          if (this.form.isEdit) {
            await this.$store.dispatch(
              "features/finance/receivable/receivable_transaction/actUpdate",
              this.form.model
            );
            var response = this.$store.state.features.finance.receivable
              .receivable_transaction.resultUpdate;
            if (response.status != 200) {
              this.handleToast(
                "Confirmation",
                "danger",
            ...
            await this.$store.dispatch(
              "features/finance/receivable/receivable_transaction/actCreate",
              this.form.model
            );

            var response = this.$store.state.features.finance.receivable
              .receivable_transaction.resultCreate;
      ...
      async doVoidJournal(row) {
        ...
        await this.$store.dispatch(
          "features/finance/receivable/receivable_transaction/actVoid",
          this.form.model
        );
        var response = this.$store.state.features.finance.receivable
          .receivable_transaction.resultPosting;
      ...
      async doPostingJournal(row) {
        ...
        await this.$store.dispatch(
          "features/finance/receivable/receivable_transaction/actPosting",
          this.form.model
        );
      ...
      async calculateFooter(){
        ...
        let footerTaxes = cloneDeep(this.form.model.ReceivableTaxes);
        ...
        this.form.model.ReceivableTaxes = footerTaxes;
